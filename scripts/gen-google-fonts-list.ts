...
import createHarfbuzz, {hbTag} from '../c-libs-wrapper/hb-wrapper';
...
const hb = await createHarfbuzz(path.resolve(import.meta.dirname, '../c-libs-wrapper/hb.wasm'));
    ...
    for (const font of f.fonts) {
        ...
        if (!f.languages) {
            ...
            for (let i = 0; i < languagesData.length; i++) {
                ...
                /*if (scratchSet.size() > 0 && scratchSet.size() < 3) {
                    let scratchStr = '';
                    for (const codePoint of scratchSet) {
                        scratchStr += String.fromCodePoint(codePoint);
                    }
                    console.log(scratchSet.size(), scratchStr, languageData.id, f.name);
                }*/
            }
        }
